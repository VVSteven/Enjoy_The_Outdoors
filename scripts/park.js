document.addEventListener("DOMContentLoaded", () => {
  ...
  displayNationalParkInformation();
  ...
  for (let typeElement of typeElements) {
    typeElement.addEventListener("click", function (event) {
      ...
      filterParks(selectedType);

      let clearAllButton = document.getElementById("clearAllButton")
      clearAllButton.innerText = "Clear ALL"

      let clearTypeButton = document.getElementById("clearTypeButton");
      clearTypeButton.innerText = selectedType;
    });
  }
  ...
  for (let stateElement of stateElements) {
    stateElement.addEventListener("click", function (event) {
      ...
      filterParks(selectedState);
      
      let clearAllButton = document.getElementById("clearAllButton")
      clearAllButton.innerText = "Clear ALL"

      let clearLocationButton = document.getElementById("clearLocationButton");
      clearLocationButton.innerText = selectedState;
    });
  }

  const clearFiltersButton = document.getElementById("clearAllButton");
  clearFiltersButton.addEventListener("click", function (event) {
    event.preventDefault();
    selectedState = "Show All";
    selectedType = "Show All";
    filterParks();
    clearFiltersButtons();
  });
  const clearTypeButton = document.getElementById("clearTypeButton");
  clearTypeButton.addEventListener("click", function (event) {
    event.preventDefault();
    selectedType = "Show All";
    filterParks();
    clearTypeButton.innerText = "";
  });
...
function filterParks() {
  ...
  console.log(selectedState);
  if (selectedState && selectedState != "Show All") {
    filterStates = filterStates.filter((f) => f.State == selectedState);
    console.log(selectedState);
  }

  if (selectedType && selectedType != "Show All") {
    filterStates = filterStates.filter((f) =>
      f.LocationName.includes(selectedType)
    );
    console.log(selectedType);
  }
  displayFiltered(filterStates);
  let resultsNum = document.getElementById("resultsNum");
  resultsNum.innerText = filterStates.length;
}

function displayFiltered(nationalParksArray) {
...
function clearFiltersButtons() {
    
  let clearAllButton = document.getElementById("clearAllButton");
  clearAllButton.innerText = "";
